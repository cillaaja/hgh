...
def save_large_file(uploaded_file, save_path):
    CHUNK_SIZE = 4 * 1024 * 1024  # 4 MB per tulis — ideal untuk stabilitas & kecepatan
    total = 0
    ...
    with open(save_path, "wb") as f:
        while True:
            ...
            f.write(chunk)
            total += len(chunk)
            # Update progress bar (tidak pakai ukuran total, Streamlit tidak tahu ukuran besar file)
            progress_bar.progress(min(1.0, total / (512 * 1024 * 1024)),  # simulasi sampai 512 MB progress
...
def main():
    ...
    st.config.set_option("server.maxUploadSize", 16000)  # 16 GB

    st.title("🎥 Live Streaming ke YouTube (Cepat & Stabil Upload)")
    ...
    if uploaded_file:
        ...
        st.info("⏳ Sedang menyimpan file besar ke disk... mohon tunggu sebentar.")
